...
char initBetas[MAX_STRING];

// All synapses and exponential table
// Initembedding for retrofitting
real *syn0, *syn1, *syn1neg, *initembed, *expTable, *betas;
clock_t start;
...
void InitNet() {
  ...
  //Load 'word' specific betas
  if(initBetas[0] != 0) {
    printf("Load betas from... %s\n", initBetas);
    long long n_betas, i;
    char *word;
    double t_beta;

    word = (char*)malloc(MAX_STRING * sizeof(char));
    a = posix_memalign((void**)&betas, 128, (long long)vocab_size * 1 * sizeof(real));
    for(a = 0; a < vocab_size; a++) {
        betas[a] = 1;
    }

    FILE *beta_file = fopen(initBetas, "rb");
    fscanf(beta_file, "%lld", &n_betas);
    for(i = 0; i < n_betas; i++){
        fscanf(beta_file, "%s %lf", word, &t_beta);
        int index = SearchVocab(word);
        betas[index] = t_beta;
        //printf("Beta %d(%s) = %f\n", index, word, betas[index]);
    }
    free(word);
  }
  ...
  while (1) {
    ...
    if (cbow) {  //train the cbow architecture
      ...
      for (a = b; a < window * 2 + 1 + sentence_vectors - b; a++) if (a != window) {
        ...
        if(betas != NULL) {
            for (c = 0; c < layer1_size; c++) syn0[c +l1] += alpha * betas[last_word] * (initembed[c + l1] - syn0[c + l1]);
            //printf("l1: %lld, beta: %f\n",last_word,betas[last_word]);
        } else {
            for (c = 0; c < layer1_size; c++) syn0[c + l1] += alpha * beta * (initembed[c + l1] - syn0[c + l1]);
        }
        //for (c = 0; c < layer1_size; c++) syn0[c + l1] += beta * (initembed[c + l1] - syn0[c + l1]);
      }
    }
    sentence_position++;
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\nExamples:\n");
    printf("./retrofit_word2vec_one -train data.txt -init initFile -output vec.txt -size 200 -window 5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -cbow 1 -iter 3\n\n");
    return 0;
  }
  output_file[0] = 0;
  save_vocab_file[0] = 0;
  ...
  if ((i = ArgPos((char *)"-beta-file", argc, argv)) > 0) strcpy(initBetas, argv[i + 1]);
